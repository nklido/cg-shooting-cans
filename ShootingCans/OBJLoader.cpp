...
GeometricMesh* OBJLoader::load(const char* filename)
{
	...
	mesh->objects.back().end = mesh->vertices.size();
...
glm::ivec3 OBJLoader::read_face_component(const char* buff, int& offset)
{
	...
	{
		v += v < 0 ? shared_vertices.size() : -1;
		vn += vn < 0 ? shared_normals.size() : -1;
		vt += vt < 0 ? shared_textcoord.size() : -1;
